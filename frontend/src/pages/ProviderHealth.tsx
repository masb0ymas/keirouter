...
import { HealthScoreRing, HealthStatusBadge, fmtIssue } from "../components/HealthBadge";
import {
  ErrorBreakdownChart,
  ErrorRateChart,
  FallbackChart,
...
function SummaryCards({ summary }: { summary: HealthSummary }) {
  const total =
    summary.healthy + summary.degraded + summary.unhealthy + summary.unknown + summary.disabled;
  const segments = [
    ...
    <Card className="p-4">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex shrink-0 gap-5 border-t border-[var(--border)] pt-3 lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0">
          <CompactStat
            label="Fallbacks"
            value={summary.fallbacks.toLocaleString()}
            tone={summary.fallbacks > 0 ? "warning" : "muted"}
          />
    ...
    <Card>
      ...
      {rows.length === 0 ? (
        <EmptyState
          ...
          hint="Run a probe or send traffic through a provider."
        />
      ) : (
        <>
...
function ModelTable({
  ...
  query: UseQueryResult<{ models: HealthModelRow[] } | HealthProviderDetail>;
}) {
  ...
  if (rows.length === 0)
    return (
      <Card>
        <EmptyState title="No model health data yet." />
      </Card>
    );
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {paged.map((r) => (
              <tr
                key={r.chain_id}
                className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--bg-subtle)] cursor-pointer"
                onClick={() => onSelect(r.chain_id)}
              >
                <td className="px-4 py-2.5 font-medium">{r.name}</td>
                <td className="px-4 py-2.5">
                  <HealthStatusBadge status={r.status} issue={r.main_issue} />
                </td>
                ...
                <td className="px-4 py-2.5 text-[var(--text-muted)]">
                  {r.affected_provider ? `${r.affected_provider}/${r.affected_model}` : "—"}
                </td>
  ...
  return (
    <Card>
      ...
      {q.isLoading ? (
        ...
        <EmptyState title="Failed to load chain detail." />
      ) : q.data ? (
        <div className="px-4 py-3">
          <div className="mb-3 flex flex-wrap items-center gap-3 text-sm">
            <span className="font-medium">{q.data.name}</span>
            <Badge tone="neutral">{q.data.strategy}</Badge>
            {q.data.requests != null && (
              <span className="text-[var(--text-muted)]">
                {q.data.requests.toLocaleString()} requests
              </span>
            )}
            {q.data.fallback_rate != null && (
              <span className="text-[var(--text-muted)]">
                {fmtPct(q.data.fallback_rate)} fallback rate
              </span>
            )}
            {q.data.final_failure_count != null && q.data.final_failure_count > 0 && (
              <span className="text-[color:var(--color-danger)]">
                {q.data.final_failure_count} final failures
              </span>
            )}
          </div>
          <div className="space-y-2">
            {q.data.steps.map((step) => (
              ...
              >
                <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-[var(--bg-subtle)] text-xs font-medium">
                  {step.position + 1}
                </span>
            ...
            {q.data.fallback_provider && (
              <div className="flex items-center gap-3 rounded-lg border border-dashed border-[var(--border)] px-3 py-2">
                <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-[var(--bg-subtle)] text-xs">
                  ★
                </span>
...
function ProbeHistoryTable({ range }: { range: string }) {
  ...
  if (rows.length === 0)
    return (
      <Card>
        <EmptyState title="No probes yet." hint="Run a manual probe to see results here." />
      </Card>
    );
  return (
    <Card>
          ...
          <tbody>
            {rows.map((r: HealthProbeRow, i) => (
              <tr key={i} className="border-b border-[var(--border)] last:border-0">
                ...
                <td className="px-4 py-2.5 text-[var(--text-muted)]">
                  {fmtIssue(r.error_type) || "—"}
                </td>
                <td className="px-4 py-2.5">
                  <Badge tone="neutral">{r.triggered_by}</Badge>
                </td>
              </tr>
            ))}
          </tbody>
    ...
    <div>
      <Card className="mb-4 p-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-2">
                ...
                {d.main_issue && (
                  <span className="text-xs text-[var(--text-muted)]">{fmtIssue(d.main_issue)}</span>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-3 border-t border-[var(--border)] pt-3">
          ...
          <MetricPill
            label="Success"
            value={fmtPct(d.metrics.success_rate)}
            tone={d.metrics.success_rate >= 95 ? "good" : "warn"}
          />
          <MetricPill
            label="Errors"
            value={fmtPct(d.metrics.error_rate)}
            tone={d.metrics.error_rate >= 5 ? "bad" : "muted"}
          />
          <MetricPill label="p95" value={fmtMs(d.metrics.latency_p95_ms)} />
          <MetricPill label="TTFT" value={fmtMs(d.metrics.ttft_p95_ms)} />
          <MetricPill
            label="Fallbacks"
            value={d.metrics.fallback_count.toLocaleString()}
            tone={d.metrics.fallback_count > 0 ? "warn" : "muted"}
          />
        </div>
      </Card>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3">
            ...
            <ManualProbeInline
              provider={provider}
              models={d.models.map((m) => m.model).filter(Boolean)}
            />
          </div>
    ...
    return (
      <Card className="mt-4">
        <EmptyState
          ...
          hint="Snapshots appear after traffic flows through this provider."
        />
      </Card>
    );
  }
        ...
        <div className="text-sm">
          {issue && (
            <p className="font-medium">
              Main issue: <span className="text-[var(--text-muted)]">{fmtIssue(issue)}</span>
            </p>
          )}
